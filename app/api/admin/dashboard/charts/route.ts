...
export async function GET(_req: NextRequest) {
  ...
  try {
    const data = await withCache("dashboard:charts:v2", TTL.SHORT, async () => {
      const now = new Date();
      ...
      // ── 3. Top 5 products (fulfilled orders only) ──────────────────────────
      ...
      // ── 4. Weekly orders: last 7 days (exclude cancelled/refunded) ─────────
